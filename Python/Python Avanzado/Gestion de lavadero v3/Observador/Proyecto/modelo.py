    ...
    def alta_orden(self, nombre, telefono, tipo, cantidad, fecha, horario, precio):
        try:
            ...
            if not re.match(r'^\d+$', str(telefono)) or telefono == '':
                ...
                self.notificar_observadores("alta modelo", nueva_orden)
...
if __name__ == "__main__":
    op = OperacionesCRUD()
    obervador_modelo = ObservadorABM()
    op.agregar_observador(obervador_modelo)
    print(op.self._observadores)
    
    op.alta_orden("asdasdasd","123123","secado",2,"21/11","Tarde","44123")
    #op.borrar_orden(29)
    #op.modificar_orden(30, "cdddd",1233,"completo",2,"09/11","Tarde",3400)
    #op.consulta_ordenes()
