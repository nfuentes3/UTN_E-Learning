...
from modelo import OperacionesDB
import re
from estilos import *
...
class MainVentana:
    ...
    def borrar(self,tree):
        ...
        try:
            if id_orden == "":
                ...
                self.actualizar_tree()
                print(f"Orden eliminada: {id_orden}")
    ...
    def modificar(self,tree):
        ...
        if id_orden == "":
            ...
            mensaje_error = Label(self.master, text=f"La orden {id_orden} fue modificada", fg=COLOR_INFO, font=TEXT_INFO)
            mensaje_error.place(x=320, y=763)
            self.master.after(6000, lambda label:label.destroy(), mensaje_error)
            self.actualizar_tree()
